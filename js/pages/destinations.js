...
export function init() {
  ...
  try {
    ...
    console.error('Error in initializeDestinationsPage:', error);
    // Show error message to user
    const container = document.getElementById('destinations-container');
    if (container) {
      container.innerHTML = '<p class="text-red-600 text-center">Failed to load destinations. Please try again later.</p>';
    }
  }
}
...
async function initializeDestinationsPage() {
  ...
  const clearAllBtn = document.querySelector('button.text-teal-400');
  ...
  // Function to get the correct city URL based on current page location
  function getCorrectCityUrl(destination) {
    // Extract the city ID from the destination URL
    const cityId = destination.url.split('id=')[1];
    ...
    if (isInPagesDir) {
      ...
      return `city.html?id=${cityId}`;
    } else {
      // We're in root directory, use path with pages prefix
      return `pages/city.html?id=${cityId}`;
    }
  }
  ...
  function createDestinationCard(destination) {
    ...
    card.innerHTML = `
      ...
      <div class="card-content p-6">
        ...
        <a href="${getCorrectCityUrl(destination)}"
  ...
  function createRecentlyViewedCard(destination) {
    ...
    card.innerHTML = `
      ...
      <div class="p-3">
        ...
        <a href="${getCorrectCityUrl(destination)}" class="text-teal-400 dark:text-cyan-400 text-xs hover:underline">View again</a>
      </div>
    `;
  ...
  function handleFilterTagClick(e) {
    ...
    if (checkbox.checked) {
      ...
      ripple.className = 'absolute inset-0 bg-teal-400 opacity-10 rounded-xl';
